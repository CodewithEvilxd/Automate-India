  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-[#030712] text-slate-900 dark:text-white transition-colors duration-300 flex flex-col">
      ...
      <main className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-10 w-full flex-1">
        {/* Header */}
        <div className="border-b border-slate-200/80 dark:border-white/10 pb-6 mb-8 text-center sm:text-left">
        ...
        <div className="rounded-2xl glass-panel p-6 mb-8 border border-slate-200/80 dark:border-white/10 shadow-xl">
          <form
            onSubmit={(e) => {
              e.preventDefault();
              handleSearch();
          ...
          >
            <div className="relative flex-1">
              ...
              <input
                ...
                className="w-full bg-slate-100 dark:bg-white/[0.04] border border-slate-200 dark:border-white/10 focus:border-cyan-500 rounded-xl pl-10 pr-4 py-3 font-mono text-xs text-slate-900 dark:text-white placeholder:text-slate-400 dark:placeholder:text-slate-500 outline-none transition-all shadow-inner"
              />
            </div>
            <button
              type="submit"
              className="px-6 py-3 bg-gradient-to-r from-cyan-500 via-teal-500 to-emerald-500 hover:from-cyan-400 hover:to-emerald-400 text-slate-950 font-display text-xs font-bold uppercase tracking-wider rounded-xl transition-all shrink-0 flex items-center justify-center gap-2 shadow-lg shadow-cyan-500/20 hover:scale-105"
            >
          ...
          <div className="mt-4 flex flex-wrap items-center gap-2 text-xs text-slate-500 dark:text-slate-400">
            ...
            {sampleQueries.map((sample, idx) => (
              <button
                ...
                className="px-2.5 py-1 rounded-lg bg-slate-100 dark:bg-white/5 hover:bg-slate-200 dark:hover:bg-white/10 border border-slate-200 dark:border-white/10 text-slate-700 dark:text-slate-300 text-xs font-medium transition-all truncate max-w-[220px]"
              >
                {sample.label}
              </button>
            ))}
        ...
        {searched && result && (
          <div className="space-y-6">
            <div className="rounded-3xl glass-panel p-6 sm:p-8 border border-emerald-500/40 shadow-2xl relative overflow-hidden">
              <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 border-b border-slate-200/60 dark:border-white/10 pb-5 mb-5">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                <div className="p-5 rounded-2xl bg-slate-100/80 dark:bg-white/[0.03] border border-slate-200/80 dark:border-white/5 space-y-3 text-xs">
                  <span className="text-[10px] uppercase tracking-widest font-mono text-slate-400 dark:text-slate-500 block font-bold border-b border-slate-200 dark:border-white/5 pb-1">
                  ...
                  <div className="flex justify-between items-center">
                    ...
                    <div className="flex items-center gap-1.5">
                      <a
                        ...
                        target="_blank"
                        rel="noreferrer"
                        className="text-cyan-600 dark:text-cyan-400 font-mono font-bold hover:underline truncate max-w-[150px] sm:max-w-[200px]"
                      >
                        {result.txHash}
                      </a>
                  ...
                  <div className="flex justify-between items-center">
                    <span className="text-slate-500 dark:text-slate-400">From Wallet (Origin):</span>
                    <WalletBadge address={result.fromWallet} />
                  </div>

                  {result.toWallet && (
                    <div className="flex justify-between items-center">
                      <span className="text-slate-500 dark:text-slate-400">To Wallet (Buyer):</span>
                  ...
                  <div className="flex justify-between items-center">
                    ...
                    <span className="font-mono text-slate-800 dark:text-slate-200">MaterialRegistry.sol</span>
                  </div>
                ...
                <div className="p-5 rounded-2xl bg-slate-100/80 dark:bg-white/[0.03] border border-slate-200/80 dark:border-white/5 space-y-3 text-xs">
                  ...
                  <div className="flex justify-between items-center">
                    <span className="text-slate-500 dark:text-slate-400">Physical Lot Mass:</span>
                  ...
                  <div className="flex justify-between items-center">
                    <span className="text-slate-500 dark:text-slate-400">Calculated CO₂ Abatement:</span>
                  ...
                  <div className="flex justify-between items-center">
                    <span className="text-slate-500 dark:text-slate-400">Location Hub:</span>
                    <span className="text-slate-800 dark:text-slate-200">{result.material?.location || "Noida, UP"}</span>
                  </div>

                  <div className="flex justify-between items-center">
                    <span className="text-slate-500 dark:text-slate-400">EPR Credit Token:</span>
                    <span className="font-mono font-bold text-cyan-600 dark:text-cyan-400">CIRC (Minted On-Chain)</span>
                  </div>
                </div>
              </div>

              {/* Fraud Risk Sentinel Badge */}
              <div className="mb-5">
                <FraudSentinelBadge
              ...
              <div className="p-4 rounded-xl bg-slate-100 dark:bg-white/[0.02] border border-slate-200/60 dark:border-white/5 text-xs text-slate-700 dark:text-slate-300 leading-relaxed italic mb-5">
              ...
              <div className="flex flex-col sm:flex-row items-center justify-between gap-3 pt-3 border-t border-slate-200/60 dark:border-white/10">
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="inline-flex items-center gap-1.5 text-xs text-cyan-600 dark:text-cyan-400 hover:underline font-mono"
                >
                ...
                <Link
                  ...
                  className="inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-slate-900 text-white dark:bg-white/10 dark:hover:bg-white/20 text-xs font-semibold transition-all shadow-sm"
                >
        ...
        {searched && !result && (
          <div className="p-10 rounded-2xl glass-panel border border-amber-500/40 text-center">
            ...
            <p className="text-slate-500 dark:text-slate-400 text-xs font-mono mt-1 max-w-md mx-auto">
